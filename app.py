...
st.set_page_config(
    page_title="Lojistik & Kargo Şube Performans Paneli",
    page_icon="📦",
    layout="wide"
)
...
with tab_main:
    ...
    # Toplam Değerler
    toplam_zimmet = df["Zimmet"].sum()
    toplam_teslimat = df["Teslim"].sum()
    toplam_devir = df["Devir"].sum()
    
    # Günün Personeli (Teslimat oranı en yüksek olan)
    ...
    with chart_col2:
        ...
        toplam_sms = df["Sms"].sum()
        toplam_imza = df["İmza"].sum()
...
# ==========================================
# 2. PERSONEL HESAP ALIMI EKRANI (Sabit Modül)
# ==========================================
with tab_personel:
    st.header("👤 Personel Hesap Alımı Ekranı")
    st.write("Personel hesap kapama işlemleri ve saha tahsilat verileri burada yönetilir.")
    st.dataframe(df[["Kurye", "Zimmet", "Teslim", "Devir", "Nakit", "Kart"]], use_container_width=True)

# ==========================================
# 3. F4 ÖDEME LİSTESİ EKRANI (Sabit Modül)
# ==========================================
with tab_f4:
