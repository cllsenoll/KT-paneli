...
FIRMA_PERSONEL_MAP = {
    ...
    "EKSAGATE ELEKTRONİK MÜHENDİSLİK VE BİLGİSAYAR SANAYİ TİCARET ANONİM ŞİRKETİ": "HASAN SAĞLAM",
    ...
    "MOGA DERİ MOBİLYA AHŞAP SANAYİ VE TİCARET LİMİTED ŞİRKETİ": "ALATTİN CEBECİ",
    ...
    "MUVA TEKSTİL SANAYİ VE TİCARET LİMİTED ŞİRKETİ": "MEHMET KAYMAZ",
    "NARVİN TEKSTİL EMLAK KOZMETİK SOSYAL MEDYA İHRACAT İTHALAT SANAYİ VE TİCARET LİMİTED ŞİRKETİ": "MEHMET KAYMAZ",
    "NEFES DERİ TEKSTİL OTOMOTİV SANAYİ VE TİCARE T LİMİTED ŞİRKETİ": "ALATTİN CEBECİ",
    "NOVMA KİMYA SANAYİ TİCARET LİMİTED ŞİRKETİ": "ALATTİN CEBECİ",
    "SELFİE TARIMSAL TEDARİK SERACILIK DEPOCULUK DANIŞMANLIK SANAYİ VE TİCARET LİMİTED ŞİRKETİ": "AHMET BERKAN ÖKSÜZ",
...
# --- FİRMA VE SORUMLU PERSONEL BULUCU ---
def match_firma_and_personel(musteri_adi):
    norm_m = normalize_text(musteri_adi)
    if not norm_m:
        return None, None
        
    for firma_adi, personel in FIRMA_PERSONEL_MAP.items():
        norm_f = normalize_text(firma_adi)
        if norm_f in norm_m or norm_m in norm_f:
            return firma_adi, personel
...
with st.sidebar:
    ...
    st.markdown("---")
    st.header("🏢 Tanımlı Firmalar & Sorumlular")
    with st.expander("Firma-Personel Eşleşmelerini Göster"):
        for f, p in FIRMA_PERSONEL_MAP.items():
            st.write(f"• **{f}** ➔ {p}")
...
if uploaded_files:
        ...
        try:
            ...
            if df_raw is not None and not df_raw.empty:
                ...
                if is_pure_f4_file:
                    ...
                    for _, row in df.iterrows():
                        musteri_val = str(row["musteri_adi"])
                        if not musteri_val or musteri_val.lower() in ["nan", "none", "null"]:
                            continue

                        matched_firma, matched_personel = match_firma_and_personel(musteri_val)
                        
                        # Tanımlı firmalardan biriyle eşleşirse işleme al
                        if matched_firma and matched_personel:
                            borc_val = float(row["fatura_borcu"])
                            aciklama_val = str(row["aciklama"])

                            tum_f4_listesi.append({
                                "Personel": matched_personel,
                                "Müşteri Adı": matched_firma,
                                "Fatura Borcu (₺)": borc_val,
                                "Açıklama": aciklama_val
                            })

                            if matched_personel not in st.session_state.personeller:
                                st.session_state.personeller.append(matched_personel)
    ...
    st.success("✅ Yüklenen dosyalar ayrıştırıldı ve koddaki tanımlı firma-personel eşleşmelerine göre aktarıldı!")
...
# ==========================================
# 4. F4 ÖDEME LİSTESİ (PERSONEL SÜZGEÇLİ VE TANIMLI FİRMA DESTEKLİ)
# ==========================================
st.subheader("📋 F4 Ödeme Listesi")

if personel_listesi:
    f4_personel_secim = st.selectbox("Tahsilatı Sağlayacak Personeli Seçin:", personel_listesi, key="f4_personel_select")
    ...
    if not df_tahsilat.empty and "Personel" in df_tahsilat.columns:
        ...
        if not p_f4_df.empty:
            ...
            st.markdown(f"**{f4_personel_secim}** Adına Tanımlı **F4 Tahsilat Listesi**:")
            st.dataframe(df_f4_goster, use_container_width=True)

            toplam_f4_borc = df_f4_goster["Fatura Borcu (₺)"].sum()
            st.info(f"💰 **{f4_personel_secim} Toplam Tahsil Edilecek Fatura Borcu:** {toplam_f4_borc:,.2f} ₺")
            ...
            with col_pdf:
                pdf_bytes = generate_pdf_bytes(df_f4_goster, f4_personel_secim)
                st.download_button(
                    label="📄 Tahsilat Listesini PDF Olarak Yazdır / İndir",
                    data=pdf_bytes,
                    file_name=f"F4_Tahsilat_Listesi_{f4_personel_secim.replace(' ', '_')}.pdf",
                    mime="application/pdf"
                )
            ...
            with col_excel:
                excel_csv = df_f4_goster.to_csv(index=True, encoding='utf-8-sig')
                st.download_button(
                    label="📥 Tahsilat Listesini Excel/CSV İndir",
                    data=excel_csv,
                    file_name=f"F4_Tahsilat_Listesi_{f4_personel_secim.replace(' ', '_')}.csv",
                    mime="text/csv"
                )
        else:
            st.warning(f"⚠️ {f4_personel_secim} için tanımlı firmalara ait F4 borç kaydı bulunamadı.")
    else:
        st.info("Henüz F4 Ödeme Listesi içeren bir Excel dosyası yüklenmedi.")
